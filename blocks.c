...
void blocks_init(const char *image_path) {
  ...
  bitmap_put(bbm, 0, 1);
  // also put bitmap for inode block?
  bitmap_put(bbm, 1, 1); // for inode blocks
}
...
int alloc_block() {
  ...
  for (int ii = 1; ii < BLOCK_COUNT; ++ii) {
    if (!bitmap_get(bbm, ii)) { // if bit at ii has a free block
      bitmap_put(bbm, ii, 1);
...
// Get a pointer to the beggining of the inode block.
void * get_inode_blocks() {
    return blocks_get_block(1);
}

// Get a pointer to the beginning of the iblock block.
void* get_iblock_blocks() {
    return blocks_get_block(2);
}

// Get a iblock at the given position
void * get_iblock(int idx) {
    void* iblockAddr = get_iblock_blocks();
    return iblockAddr + BLOCK_SIZE * idx;
}
